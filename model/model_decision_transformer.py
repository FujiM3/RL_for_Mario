...
class DecisionTransformerAttention(nn.Module):
    ...
    def forward(
        ...
        xq = xq.transpose(1, 2)
        xk = repeat_kv(xk_hf.transpose(1, 2), self.n_rep).transpose(1, 2)
        xv = repeat_kv(xv_hf.transpose(1, 2), self.n_rep).transpose(1, 2)
        ...
        if self.flash and (seq_len > 1) and (past_key_value is None) and (
            ...
            if attention_mask is not None:
                ...
                scores = scores + (1.0 - extended_attention_mask) * -1e9
...
class MarioStateEncoder(nn.Module):
    def __init__(self, state_shape: Tuple[int, int, int], hidden_size: int):
        ...
        self.proj = nn.Linear(c * h * w, hidden_size)
    ...
    def forward(self, states: torch.Tensor) -> torch.Tensor:
        ...
        if states.max() > 1.0:
            states = states / 255.0
        return self.proj(self.flatten(states))
...
class DecisionTransformerModel(nn.Module):
    def __init__(self, config: DecisionTransformerConfig):
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.layers = nn.ModuleList([DecisionTransformerBlock(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        if states.ndim != 5:
            raise ValueError(f"states must be [B,T,C,H,W], got {tuple(states.shape)}")
        if states.shape[2:] != self.config.state_shape:
            raise ValueError(
                f"states spatial shape must match config.state_shape={self.config.state_shape}, got {tuple(states.shape[2:])}"
            )
        if actions is None or actions.shape[:2] != (batch_size, seq_len):
            raise ValueError(f"actions must be [B,T], got {None if actions is None else tuple(actions.shape)}")
        ...
        returns_embeddings = self.embed_return(returns_to_go.unsqueeze(-1).float()) + time_embeddings
        action_embeddings = self.embed_action(actions.long().clamp(0, self.config.act_dim - 1)) + time_embeddings
        ...
        if attention_mask is not None:
            ...
            ).reshape(batch_size, seq_len * 3).to(hidden_states.dtype)
        ...
        hidden_states = self.norm(hidden_states).view(batch_size, seq_len, 3, self.config.hidden_size)
...
class MarioDecisionTransformer(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        action_logits = self.action_head(state_tokens)
        value_preds = self.value_head(state_tokens).squeeze(-1)
    ...
    def act(
        ...
        logits = outputs.action_logits[:, -1, :]
        if deterministic:
